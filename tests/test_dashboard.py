...
class DashboardTest(VCRTestCase):
    ...
    def test_post_register_name_dispute(self):
        response = self.client.post_register_name_dispute(test_session, "test-snap", "testing a dispute")
    ...
    def test_get_package_upload_macaroon(self):
        macaroon = self.client.get_package_upload_macaroon(test_session, "test-snap", ["testing"])
    ...
    def test_get_snap_revision(self):
        revision = self.client.get_snap_revision(test_session, "steve-test-snap", 22)
        self.assertIsInstance(revision, dict)
        self.assertIn("revision", revision)

    def test_snap_release_history(self):
        release_history = self.client.snap_release_history(test_session, "steve-test-snap")
    ...
    def test_snap_channel_map(self):
        channel_map = self.client.snap_channel_map(test_session, "steve-test-snap")
    ...
    def test_post_snap_release(self):
        release = self.client.post_snap_release(test_session, "steve-test-snap", {"channels": ["edge"], "name": "steve-test-snap", "revision": 22})
        self.assertIsInstance(release, dict)

    def test_post_close_channel(self):
        close_channel = self.client.post_close_channel(test_session, "bv9Q2i9CNAvTjt9wTx1cFC6SAT9YrEfG", {"channels": ["edge"]})
    ...
    def test_get_store_snaps(self):
        snaps = self.client.get_store_snaps(test_session, "testGahNg0wohVae8aY4")
        self.assertIsInstance(snaps, list)

    def test_get_store_members(self):
        members = self.client.get_store_members(test_session, "testGahNg0wohVae8aY4")
        self.assertIsInstance(members, list)
        self.assertNotEqual(len(members), 0)

    def test_get_store_invites(self):
        invite = self.client.get_store_invites(test_session, "testGahNg0wohVae8aY4")
